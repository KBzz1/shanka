...
| Markdown 单文件资料（2026-09-20，V25-D-40） | `DONE`（后端+契约+前端代码；模拟器/真机验收未登记） | 单独 `.md` 文件直接上传作制卡资料：`POST /projects/{id}/materials/markdown`（multipart，≤20MB、正文≤30 万字、UTF-8）同步解析即时就绪，零新依赖纯标准库（`services/projects/markdown_doc.py`）。分诊复用 V25-D-38 同款：最浅出现的 ATX 标题级（`#`~`######`，行首、围栏代码块内不计）出现 ≥2 次 → 每标题一章节（source=HEADING，chunk_seq 伪页码区间，复用 HTML 段落章节持久化）；无标题/标题级仅 1 个/总字数≤`single_chapter_max_chars` → 恒单章（source=AUTO 零模型）；YAML frontmatter（首行 `---` 至闭合）丢弃；Material(type=MARKDOWN, READY) 不存档文件本体；错误码 `MARKDOWN_UPLOAD_INVALID`(400)/`MARKDOWN_EXTRACT_FAILED`(422)；限流并入文件材料共享桶（10 次/时/user）。契约同步（openapi 新路径+MaterialType/ChapterSource/MaterialStatus 枚举、structure-contract 3.2a/3.3/§6 端点行/§7 错误码/§1 限流行、database-design 2.22 type 域（顺手补 HTML 漏记）、frontend/backend-integration §3.3）。前端：V25Api/仓库（Remote+OfflineFirst）/V25MaterialType 枚举、MaterialUpload.Markdown + PdfUploadOperation.AddMarkdownMaterial、AppViewModel 识别 `.md`/`.markdown` + commitStagedMarkdown + 项目创建/导入两条上传链、MaterialImportScreen 选择器 MIME 加 text/markdown+text/plain、ErrorMessages 两个新码本地化。验证（2026-09-20）：后端新增 unit 6 + integration 3 测试，全量 pytest 982/982 + ruff/mypy strict 绿；前端 compileDebugKotlin + testDebugUnitTest 绿。发布顺序：**后端先发 → APK 后发**（旧 APK 不受影响，仅看不到 md 入口） |
| ZIP 扁平笔记独立成章（2026-09-20，V25-D-41） | `DONE`（后端+契约；前端零改动——章节服务端派生） | 修订 V25-D-35 ZIP 章节规则：主文件夹下**没有任何含 md 的子文件夹**时，每个根级 md 文件独立成章（章名 = 文件名去 `.md` 扩展，自然排序），不再合并进单一「总览」章；混合结构（含 md 的子文件夹 + 根级 md）保持原语义（根级 md 合收「总览」排最前）；整树无 md 的空子文件夹不构成章节也不阻止根级 md 成章。判定以"含 md 的子文件夹"为准（结构不可见的空文件夹不改变行为）。改动点：`services/projects/zip_archive.py` `_collect` 章节组装 + `_md_stem`；契约同步（openapi zip 文件描述、structure-contract §6 zip 端点行、frontend/backend-integration 补 zip 端点行）。验证（2026-09-20）：新增单测 3（扁平多 md 成章自然排序/单 md 文件名成章/空子文件夹不阻止），test_zip_archive 19/19 + test_projects_api 全过 + 契约守卫 104/104 + ruff/mypy 绿。既有已入库 ZIP 资料章节不受影响（章节为上传时快照） |
| 连胜火苗吸收（2026-09-25，V25-D-42） | `DONE`（后端+契约+前端代码；模拟器/真机验收未登记） | 修订历史连胜清零语义（用户核心诉求：断卡一天不清零，火苗=复活消耗品）：火苗每 2 个连续计数日攒 1 个、上限 5（6 天=3 个）；已结束空缺日自动消耗 1 个火苗跳过（不计天数、连胜不断卡），火苗耗尽才清零重开；连续多个空缺日逐个消耗；**今天尚未复习视为"待打"**——既不清零也不耗火苗（修复旧实现早晨直接显示 0）；火苗须在空缺日之前挣得（先攒后用）、不跨 run 结转。纯推导无新表（前向单遍扫描 review_events，符合 database-design"不建第二套状态"）。看板新增 `streak_flames_available`/`streak_flames_used`/`max_streak_days`（真·历史最长，顺带修 DataScreen 拿当前连胜冒充"单次最大连胜"）。后端：`services/stats/service.py` `_streak_and_flames` 前向推导；契约同步（structure-contract §3.12 三字段+streak_days 语义改写、openapi StatsDashboard、PRD §0.3 替换索引行 + V25-D-42 决策条目——语义变更属破坏性须同步 PRD）；打卡口径不变（只有复习评级算打卡，自由刷题不计）。前端：DashboardDto/V25Models/Room `dashboard_snapshot` 加列（Migration 9→10，schemas 10.json）、HomeScreen 5 槽轨道改投可用火苗（大数字仍连胜天数）、DataScreen 改用 maxStreakDays（旧服务端 0 兜底回退当前值）。验证（2026-09-25）：后端新增单测 8（纯函数：断卡吸收/待打不清零/连断耗尽/封顶/不结转/先攒后用）+ 集成 2；全量 pytest 1019/1019 + ruff/mypy strict 绿；前端 compileDebugKotlin + testDebugUnitTest 绿（HomeProjectionTest 改锁火苗投影 + 迁移测试 v9→v10）。**兼容性：旧 APK 忽略新字段不受影响；后端先发即可** |
| 问答直通制卡模式（2026-09-25，V25-D-43） | `DONE`（后端+契约+前端代码+真实验收与双裁判盲评通过；模拟器/真机验收未登记） | 资料已含问题和答案（题库/问答集）时的直通制卡：任务配置 `source_mode=QA_DIRECT`（缺省 EXTRACT 既有语义），**八态状态机与 executor 四 worker 骨架零改动**，仅内部三阶段语义切换——规划=资料问答对提取（qa-planner 资产，operation_key 前缀 `planning:qa:`，账本 stage 仍 PLANNING 免 CHECK 迁移；每对映射一个生成单元：topic=原问题/原陈述、卡型 QUESTION/TRUE_FALSE 照录、难度统一 BASIC 归档、tier=NULL）、生成=generator-qa 整理成卡（**仅格式规范化**：用户裁决忠实度口径——不改写/不润色/不纠错/不重新命题；SPEC 携带原问题，答案在来源页定位照录、定位不唯一即弃权）、评分照常 rubric v3；样卡保留确认步（QA 模式单张 BASIC 标注，`sample:true` 样卡直取条款预览提取效果）。agent_evolution 新资产（技术评审级变更）：`prompts/v1/qa-planner.md`+`prompts/v1/generator-qa.md`+`schemas/v1/qa-planner-output.schema.json`（双形态 oneOf），manifest 增键 qa_planner/generator_qa/qa_planner_output（v1）+ CHANGELOG；`asset_versions()` 加三观测键。预算按题库密度（`qa_pairs_per_10k_chars`=40）创建期拒绝超限而非静默截断用户题库；`config_fingerprint` 归一化——EXTRACT 缺省不进指纹载荷，部署前在途任务的 sample_config_hash 保持有效；`_merge_pairs_to_units` 任务级 300 上限截断兜底。后端新增：`services/generation/qa_planning_executor.py`（复用 claim/冻结/账本/心跳/三分支终态全定式）+ `qa_planner_validator.py`（Schema 双形态/来源接地/上限/normalize_question 去重键）+ executor 规划 worker 按 source_mode 分派 + batches/samples 提示词与账本版本键切换。前端：V25SourceMode 枚举 + DTO/请求（EXTRACT 不发送 source_mode，旧服务端字节兼容）+ 新路由 `AppRoute.QaCardChapter`（非基础路由）+ 选资料页（添加卡片组）与选章节页（智能制卡）右上角"问答直通"按钮（复用 ScreenTopInformationBar trailing 槽）+ `QaCardChapterScreen`（复用章节卡组件，说明文案"AI 不重新命题、不改答案"）→ `beginPdfSamples(sourceMode=QA_DIRECT)` 走同款样卡→生成→评审链；`boundTaskSourceMode` StateFlow 供预览页切文案（轻量任务投影不携带 config，loadPdfSamples 读全量任务以服务端真值覆盖）；任务复用判定含模式（EXTRACT 任务与 QA 配置互不复用）。契约同步（openapi GenerationConfig+SourceMode 枚举、structure-contract §3.5 source_mode 行+编排分派说明、PRD V25-D-43 决策条目、backend-integration §3.5/§3.10）。验证（2026-09-25）：后端新增单测/服务层/集成 19（提取落库/去重/空产出 COMPLETED/全失败 FAILED/worker 分派/样卡直取/预算/指纹兼容）；全量 pytest 1019/1019 + ruff/mypy strict 绿 + 契约守卫 108/108；前端 compileDebugKotlin + testDebugUnitTest 绿。**真实验收（2026-09-25，新评测工作区 `scripts/qa_direct_eval/`）**：首轮跑生产 HTTP 全链暴露 qa-planner 输出回抄每对完整答案、撑爆 planner_max_output_tokens=2048 → JSON 截断三连重试失败；修复 = v1 定稿为**清单式输出**（只登记题干/陈述+卡型+出处，不回抄答案——答案由制卡阶段按出处回原文照录；schema 去 answer 类字段、校验器对模型回抄键防御性剥除；未发布原地修订，CHANGELOG/测试同步）。复跑任务 f87e893f 全链 39.7s 产 23 卡 = GT 去重后全量：召回 100%（23/23，植入重复题正确合并为 1）、自创 0、重复 0、判断题布尔 4/4（含唯一 true 项）；**双裁判盲评**（新设计忠实度 rubric v1：Q1 题干保真/Q2 答案保真/Q3 自包含可复习/Q4 卡面字段适配，与 EXTRACT rubric v3 不通用）两裁判四维均 3.00、裁判间完全一致 23/23、gt_id 映射全对；客观门禁 G1-G5 全过；C 组 1.13 调用/卡（EXTRACT 基线 ≈2）、1.7s/卡、重试 0%；题号变体（`7.`/`第 7 题：`/`（10）`）剥离与代码块答案排版均正确。证据链：`run/20260925T094247Z/`（cards/report/task_quality_report 留档，run/ 不入 Git）+ `leaderboard.md` 首行 + `rubric.md`；generation-quality-metrics.md 已挂 QA 模式评测指针章节。**兼容性：后端先发 → APK 后发**（旧 APK 不发送 source_mode 完全走既有语义） |
