...
| 当前机器契约 | `DONE`（V2.5 实现事实） | `structure-contract.md`（v2.5）、`openapi.yaml`（2.5.0，2026-09-04 路径覆盖补齐至全部 48 个业务端点，probes/metrics 按 R-04 有意排除）、`database-design.md`（v2.5）均自标 V2.5 实现事实，contract 守卫套件通过（含 app 路由 ↔ openapi 双向路径守卫） |
