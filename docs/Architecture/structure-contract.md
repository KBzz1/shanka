...
IP 维度语义(离线优先地基,token bucket):`rate_limit_ip_per_second=5` 为持续补充速率(每 0.2s 补 1 token),`rate_limit_ip_burst=10` 为桶容量——同 IP 首次可立即消费 10 个请求(吸收正常移动端启动/刷新短突发),耗尽后按持续速率恢复。`Retry-After` = 下一 token 可用时间向上取整、最少 1 秒(整数响应头兼容)。桶状态按 IP 隔离且有界(仅淘汰已回满的 idle key,不使活跃 IP 绕过持续速率)。实现为单实例内存态;多实例演进时换共享存储,业务逻辑不变(4.4 定式)。业务长窗口维度(写/API Key/samples/PDF/auth)仍为固定窗口,语义不变。
