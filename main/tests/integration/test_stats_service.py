...
- FK 强制（engine 级 PRAGMA foreign_keys=ON）：_seed_events 前置 Device/Deck/Card 行
...
from infra.db.models import Base, Card, Device, ReviewEvent, ReviewState
...
def _seed_device(session: Session, *, device_id: str) -> None:
    ...
    if session.scalar(select(Device).where(Device.device_id == device_id)) is None:
        session.add(Device(device_id=device_id, created_at="2026-01-01T00:00:00.000Z"))
        session.flush()


def _seed_events(
    session: Session, *, device_id: str, events: list[tuple[str, str, str]], same_card: bool = False
) -> None:
    """事件种子：(client_event_id, rating, reviewed_at)；FK 前置 device/deck/card。

    same_card=True 时全部事件共用一张卡（retention 非首次用例）。
    """
    _seed_device(session, device_id=device_id)
    deck = create_deck(session, device_id=device_id, name="D", now="2026-01-01T00:00:00.000Z")
    session.flush()
    card_ids: list[str] = []
...
def test_stats_dashboard_empty_has_data_false(session_factory: Callable[[], Session]) -> None:
    ...
    with session_factory() as session:
        result = dashboard(
            session, device_id=device, timezone="Asia/Shanghai", weekly_goal=None, now=_now()
        )
...
def test_stats_dashboard_weekly_bucketing_monday(session_factory: Callable[[], Session]) -> None:
    ...
    with session_factory() as session:
        result = dashboard(
            session, device_id=device, timezone="Asia/Shanghai", weekly_goal=10, now=_now()
        )
...
def test_stats_dashboard_mastered_count(session_factory: Callable[[], Session]) -> None:
    ...
    with session_factory() as session:
        ...
        deck = create_deck(session, device_id=device, name="D", now="2026-01-01T00:00:00.000Z")
        session.flush()
...
def test_stats_dashboard_invalid_timezone_rejected(session_factory: Callable[[], Session]) -> None:
    ...
    with session_factory() as session, pytest.raises(AppError) as excinfo:
        dashboard(session, device_id=device, timezone="Not/AZone", weekly_goal=None, now=_now())
