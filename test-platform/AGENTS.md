...
- V2.5 写路径复用 `shanka/client.py` 模块级请求助手（create_task/request_samples/start_task/abandon_task/retry_task/delete_task/submit_review/delete_card/pending_deletion_batches/undo_deletion_batch/create_rewrite_preview/apply_rewrite_preview/cancel_rewrite_preview，统一幂等键纪律），StubClient 同形状可共用；多客户端场景（模拟换设备/重登）在 `run(...)` 增加 `client_factory` 参数（默认按 base_url 新建 ShankaClient），供无网络测试注入 StubClient。
...
- 重写预览调用不在 derive_budget 推导模型内：含两阶段重写的生成类场景按实际预览次数显式补充计入 LLM_CALLS（`v25_core_flow` = 推导 59 + 预览 2 = 61；闸门与报告按合计计，场景 docstring 声明构成）。
