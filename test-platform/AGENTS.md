...
- 套件：`quick` = 0 次 LLM 调用纯无 Key 冒烟；`full` = 非生成场景 + api_key（合计 1 次校验调用，域场景实装后扩展）；`live` = 完整制卡流程（真实生成，经成本闸门核算：合计 3，未超默认阈值）。**live 套件需固定 `--device-id` 运行（随机设备无预置 PDF——后端按设备隔离数据，live_flow 复用已解析 PDF 须与预置数据同设备）**。
