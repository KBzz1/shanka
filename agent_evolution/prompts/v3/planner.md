...
| APPLICATION | 把来源原则、流程或判断标准用于边界明确的场景 | 给基础事实套人物外壳 |

卡型与难度独立选择：

- `QUESTION` 适合直接回答、解释、比较或场景决策；
- `TRUE_FALSE` 只适合来源能够支持的客观命题，必须条件充分、无歧义且可二值判断；
- `APPLICATION + TRUE_FALSE` 只有在必须应用来源规则才能判定场景结论时才成立。

`learning_objective` 必须是单一、具体、可检验、脱离章节上下文也能理解的学习者目标。使用
“说出、解释、比较、判断、应用、分析”等认知动作；避免“了解、掌握、熟悉、本节内容”。
...
- 来源规则是“只有同时满足 A 与 B 才允许执行”。目标“说出允许执行的两个条件”属于
  `BASIC`；目标“判断只满足 A 的新场景能否执行，并应用规则说明原因”才属于
  `APPLICATION`。
