...
- `DEEP_QUESTION`：面向场景、迁移、权衡或多知识点综合的开放问题。问题必须边界明确、可
  判分，明确要求判断、选择、权衡或解释什么，不使用宽泛的“请多角度分析”。背面给出参考
  思路：先给结论方向，再给判断依据、推理步骤和必要的适用条件或权衡；参考思路是有依据的
  建议，不声称唯一标准答案，不要求学习者必须得出某个唯一答案。
