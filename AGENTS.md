...
- `docs/PRD/V2.1/prd_v2_1.md` — 需求权威（做什么、为什么）。
- `docs/Architecture/` — 设计契约集合 + 防漂移规则，见 `docs/Architecture/AGENTS.md`。
- `main/` — FastAPI 后端（Python >= 3.12）：
  ...
  - `services/` 用例编排：pdf / generation / scheduling（FSRS-6）/ decks / stats。
  - `infra/` db（ORM 与迁移）/ storage（PDF 文件）/ llm（DeepSeek 调用、Prompt 组装）。
...
- 测试：`cd main && conda run -n shanka-backend python -m pytest`。四层职责见 `main/tests/*/AGENTS.md`；命名规范 `test_<模块>_<行为>`。
